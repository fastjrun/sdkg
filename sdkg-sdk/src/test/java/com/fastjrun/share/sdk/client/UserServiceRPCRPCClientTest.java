...
import com.fastjrun.share.sdk.client.UserServiceRPCRPCClient;
 ...
 * @author cuiyingfeng
 */
public class UserServiceRPCRPCClientTest {

    final Logger log = LogManager.getLogger(this.getClass());
    UserServiceRPCRPCClient userServiceRPCRPCClient = new UserServiceRPCRPCClient();
    ...
    public void init(String envName) {
        userServiceRPCRPCClient.initSDKConfig("apiworld");
        try {
            InputStream inParam = UserServiceRPCRPCClient.class.getResourceAsStream((("/testdata/"+ envName)+".properties"));
    ...
    public Object[][] loadParam(Method method) {
        ...
        for (String key: keys) {
            if (key.startsWith((("UserServiceRPCRPCClient"+".")+(method.getName()+".")))) {
    ...
    public void testRegister(String reqParamsJsonStr) {
        ...
        try {
            userServiceRPCRPCClient.register(registserRestRequestBody);
        } catch (Exception _x) {
            _x.printStackTrace();
        }
    }
    ...
    public void testLogin(String reqParamsJsonStr) {
        ...
        try {
            LoginRestResponseBody responseBody = userServiceRPCRPCClient.login(loginRestRequestBody);
    ...
    public void testLoginv1_1(String reqParamsJsonStr) {
        ...
        try {
            LoginRestResponseBody responseBody = userServiceRPCRPCClient.loginv1_1(loginRestRequestBody);
            log.info(responseBody);
        } catch (Exception _x) {
            _x.printStackTrace();
        }
    ...
    public void testAutoLogin(String reqParamsJsonStr) {
        ...
        try {
            LoginRestResponseBody responseBody = userServiceRPCRPCClient.autoLogin(autoLoginRestRequestBody);
            log.info(responseBody);
        } catch (Exception _x) {
            _x.printStackTrace();
        }
