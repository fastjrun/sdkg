...
public class ServiceMethodGenerator extends AbstractMethodGenerator {
    ...
    public void processServiceMethod() {
        this.jServiceMethod =
          this.serviceGenerator.getServiceClass().method(JMod.NONE, this.responseBodyClass,
            this.methodName);
        String methodRemark = commonMethod.getRemark();
        this.jServiceMethod.javadoc().append(methodRemark);

        MethodGeneratorHelper.processServiceMethodVariables(this.jServiceMethod,
          this.commonMethod.getHeadVariables(), this.cm);
        MethodGeneratorHelper.processServiceMethodVariables(this.jServiceMethod,
          this.commonMethod.getPathVariables(), this.cm);
        MethodGeneratorHelper.processServiceMethodVariables(this.jServiceMethod,
          this.commonMethod.getParameters(), this.cm);
        MethodGeneratorHelper.processServiceMethodVariables(this.jServiceMethod,
          this.commonMethod.getCookieVariables(), this.cm);
    ...
    public void processServiceTestMethod() {
        ...
        JMethod jServiceTestMethod = serviceTestClass.method(JMod.PUBLIC, cmTest.VOID,
          "test" + StringHelper.toUpperCaseFirstOne(this.methodName));
        ...
        if (headVariables != null && headVariables.size() > 0) {
            for (int index = 0; index < headVariables.size(); index++) {
                ...
                MethodGeneratorHelper.processMethodCommonVariables(cmTest, methodTestBlk,
                  reqParamsJsonJVar, headVariable);
            }
        ...
        if (pathVariables != null && pathVariables.size() > 0) {
            for (int index = 0; index < pathVariables.size(); index++) {
                ...
                MethodGeneratorHelper.processMethodCommonVariables(cmTest, methodTestBlk,
                  reqParamsJsonJVar, pathVariable);
            }
        }
        ...
        if (this.getRequestBodyClass() != null) {
            ...
            jNotNullBlock.assign(requestBodyVar,
              cmTest.ref(JacksonUtilsClassName).staticInvoke("readValue").arg(
                ...
                ((AbstractJClass) this.getRequestBodyClass()).dotclass()));
        ...
        if (this.getResponseBodyClass() != cm.VOID) {
            ...
            if (this.getCommonMethod().isResponseIsArray()) {
                ...
                JVar responseBodyIndexVar =
                  forBlock1.decl(this.getElementClass(), "item" + this.getElementClass().name(),
                    responseBodyVar.invoke("get").arg(initIndexVar));

                MethodGeneratorHelper.logResponseBody(cmTest, log, packageNamePrefix, 1,
                  this.getCommonMethod().getResponse(), responseBodyIndexVar, forBlock1);

            } else {
                MethodGeneratorHelper.logResponseBody(cmTest, log, packageNamePrefix, 1,
                  this.getCommonMethod().getResponse(), responseBodyVar, ifBlock1);
            }
        } else {
            JConditional jConditional1 = methodTestBlk._if(assertJsonJVar.ne(JExpr._null()));
            JBlock jConditional1Block = jConditional1._then();
    ...
    public void processServiceMockMethod() {
        this.jServiceMockMethod =
          this.serviceGenerator.getServiceMockClass().method(JMod.PUBLIC, this.responseBodyClass,
            this.methodName);
        String methodRemark = commonMethod.getRemark();
        this.jServiceMockMethod.javadoc().append(methodRemark);

        MethodGeneratorHelper.processServiceMethodVariables(this.jServiceMockMethod,
          this.commonMethod.getHeadVariables(), this.cm);
        MethodGeneratorHelper.processServiceMethodVariables(this.jServiceMockMethod,
          this.commonMethod.getPathVariables(), this.cm);
        MethodGeneratorHelper.processServiceMethodVariables(this.jServiceMockMethod,
            ...
            if (!commonMethod.isResponseIsArray()) {
                if (responseBodyClass.name().endsWith("Boolean")) {
                    serviceMockMethodBlock._return(
                      cm.ref(mockHelperClassName).staticInvoke("geBoolean"));
                } else if (responseBodyClass.name().endsWith("Integer")) {
                    serviceMockMethodBlock._return(
                      cm.ref(mockHelperClassName).staticInvoke("geInteger"));
                } else if (responseBodyClass.name().endsWith("Long")) {
                    serviceMockMethodBlock._return(
                      cm.ref(mockHelperClassName).staticInvoke("geLong"));
                } else if (responseBodyClass.name().endsWith("Double")) {
                    serviceMockMethodBlock._return(
                      cm.ref(mockHelperClassName).staticInvoke("geDouble"));
                } else if (responseBodyClass.name().endsWith("String")) {
                    serviceMockMethodBlock._return(
                      cm.ref(mockHelperClassName).staticInvoke("geStringWithAlphabetic").arg(
                        JExpr.lit(10)));
                } else if (responseBodyClass.name().endsWith("Date")) {
                    serviceMockMethodBlock._return(
                      cm.ref(mockHelperClassName).staticInvoke("geDate").arg(JExpr.lit(10)));
                } else {
                    JVar responseBodyVar = this.composeResponseBody(0, serviceMockMethodBlock,
                      commonMethod.getResponse(), this.elementClass);
                ...
                if (elementClass.name().endsWith("Boolean")) {
                    ...
                    serviceMockMethodBlock._return(
                      cm.ref(mockHelperClassName).staticInvoke("geStringListWithAscii").arg(
                        JExpr.lit(10)));
                    ...
                    JVar responseVar =
                      serviceMockMethodBlock.decl(this.responseBodyClass, "response",
        ...
        if (roList != null && roList.size() > 0) {
            for (String listName : roList.keySet()) {
                ...
                String varNamePrefixList = StringHelper.toLowerCaseFirstOne(
                  roListEntityClass.name().substring(
                    roListEntityClass.name().lastIndexOf(".") + 1));
                ...
                JVar iSizeVar = methodBlk.decl(cm.INT, "iSize" + listName + loopSeq + index,
                  cm.ref(mockHelperClassName).staticInvoke("geInteger").arg(JExpr.lit(10)).invoke(
                    "intValue"));
    ...
    private JVar composeResponseBodyField(int loopSeq, JBlock methodBlk, PacketObject responseBody,
      AbstractJType responseBodyClass) {
        String varNamePrefix = StringHelper.toLowerCaseFirstOne(responseBody.get_class().substring(
          responseBody.get_class().lastIndexOf(".") + 1)) + loopSeq;
        JVar reponseBodyVar =
            ...
            for (String fieldName : restFields.keySet()) {
                ...
                if (primitiveType != null) {
                    if (primitiveType.endsWith("String")) {
                        ...
                        methodBlk.add(reponseBodyVar.invoke(tterMethodName).arg(
                          (cm.ref(mockHelperClassName).staticInvoke("geBooleanList").arg(
                            JExpr.lit(10)))));
                    } else if (primitiveType.endsWith("Integer")) {
                        methodBlk.add(reponseBodyVar.invoke(tterMethodName).arg(
                          (cm.ref(mockHelperClassName).staticInvoke("geIntegerList").arg(
                            JExpr.lit(10)))));
                    } else if (primitiveType.endsWith("Long")) {
                        methodBlk.add(reponseBodyVar.invoke(tterMethodName).arg(
                          (cm.ref(mockHelperClassName).staticInvoke("geLongList").arg(
                            JExpr.lit(10)))));
                    } else if (primitiveType.endsWith("Float")) {
                        methodBlk.add(reponseBodyVar.invoke(tterMethodName).arg(
                          (cm.ref(mockHelperClassName).staticInvoke("geFloatList").arg(
                            JExpr.lit(10)))));
                    } else if (primitiveType.endsWith("Double")) {
                        methodBlk.add(reponseBodyVar.invoke(tterMethodName).arg(
                          (cm.ref(mockHelperClassName).staticInvoke("geDoubleList").arg(
                            JExpr.lit(10)))));
                    } else if (primitiveType.endsWith("Date")) {
                        methodBlk.add(reponseBodyVar.invoke(tterMethodName).arg(
                          (cm.ref(mockHelperClassName).staticInvoke("geDateList").arg(
                            JExpr.lit(10)))));
                    } else {
                    ...
                    methodBlk.add(reponseBodyVar.invoke(tterMethodName).arg(
                      (cm.ref(mockHelperClassName).staticInvoke("geStringWithAscii").arg(
                        JExpr.lit(Integer.parseInt(length))))));
                    ...
                    methodBlk.add(reponseBodyVar.invoke(tterMethodName).arg(
                      (cm.ref(mockHelperClassName).staticInvoke("geLong").arg(JExpr.lit(100)))));
                } else if (jType.name().endsWith("Float")) {
                    methodBlk.add(reponseBodyVar.invoke(tterMethodName).arg(
                      (cm.ref(mockHelperClassName).staticInvoke("geFloat").arg(JExpr.lit(100)))));
                } else if (jType.name().endsWith("Double")) {
                    methodBlk.add(reponseBodyVar.invoke(tterMethodName).arg(
                      (cm.ref(mockHelperClassName).staticInvoke("geDouble").arg(JExpr.lit(100)))));
                } else if (jType.name().endsWith("Date")) {
                    methodBlk.add(reponseBodyVar.invoke(tterMethodName).arg(
                      (cm.ref(mockHelperClassName).staticInvoke("geDate").arg(JExpr.lit(100)))));
                } else {
                    throw new CodeGException("CG504",
                      responseBodyClass.name() + "." + tterMethodName + " handled failed:for" + dataType);
                }
