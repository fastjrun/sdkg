...
public abstract class BaseControllerMethodGenerator extends BaseCMGenerator {
    ...
    public void processClientTestMethod(JDefinedClass clientTestClass) {
        ...
        JVar jsonNodes =
          methodTestBlk.decl(cmTest.ref("com.fasterxml.jackson.databind.JsonNode").array(),
            "jsonNodes", JExpr.invoke("parseStr2JsonArray").arg(reqParamsJsonStrAndAssertJVar));
        JVar reqParamsJsonJVar =
          methodTestBlk.decl(cmTest.ref(JSONOBJECTCLASS_NAME), "reqParamsJson",
            jsonNodes.component(0));
        JVar assertJsonJVar = methodTestBlk.decl(cmTest.ref(JSONOBJECTCLASS_NAME), "assertJson",
          jsonNodes.component(1));
        ...
        if (this.serviceMethodGenerator.getRequestBodyClass() != null) {
            JVar requestBodyVar =
              methodTestBlk.decl(this.serviceMethodGenerator.getRequestBodyClass(), "requestBody",
                JExpr._null());
            JVar requestBodyStrVar =
              methodTestBlk.decl(cmTest.ref(JSONOBJECTCLASS_NAME), "reqJsonRequestBody",
            ...
            jNotNullBlock.assign(requestBodyVar,
              cmTest.ref(JacksonUtilsClassName).staticInvoke("readValue").arg(
                requestBodyStrVar.invoke("toString")).arg(
        ...
        if (this.serviceMethodGenerator.getResponseBodyClass() != cm.VOID) {
            JVar responseBodyVar =
              methodTestBlk.decl(this.serviceMethodGenerator.getResponseBodyClass(), "responseBody",
                JExpr._null());

            JConditional jConditional1 = methodTestBlk._if(assertJsonJVar.ne(JExpr._null()));
            JBlock jConditional1Block = jConditional1._then();
            JVar codeNodeJVar =
              jConditional1Block.decl(cmTest.ref(JSONOBJECTCLASS_NAME), "codeNode",
            ...
            JCatchBlock jCatchBlock =
              jTry._catch(cmTest.ref("com.fastjrun.common.ClientException"));
            JVar jExceptionVar = jCatchBlock.param("e");
            JBlock jCatchBlockBody = jCatchBlock.body();
            jCatchBlockBody.add(cmTest.ref("org.testng.Assert").staticInvoke("assertEquals").arg(
              jExceptionVar.invoke("getCode")).arg(codeNodeJVar.invoke("asText")).arg(
              JExpr.lit("返回消息码不是指定消息码：").plus(codeNodeJVar.invoke("asText"))));
            jConditional2._else().assign(responseBodyVar, jInvocationTest);
            jConditional1._else().assign(responseBodyVar, jInvocationTest);

            methodTestBlk.add(
              JExpr.refthis("log").invoke("debug").arg(JExpr.lit("response={}")).arg(
                cmTest.ref(JacksonUtilsClassName).staticInvoke("toJSon").arg(responseBodyVar)));
            ...
            if (this.serviceMethodGenerator.getCommonMethod().isResponseIsArray()) {
                ...
                this.logResponseBody(1, this.serviceMethodGenerator.getCommonMethod().getResponse(),
                  responseBodyVar, ifBlock1);
            }
            ifBlock1.add(
              JExpr._this().invoke("processAssertion").arg(assertJsonJVar).arg(responseBodyVar).arg(
                JExpr.dotClass(this.serviceMethodGenerator.getResponseBodyClass())));
    ...
    private void processMethodCommonVariables(JBlock methodTestBlk, JVar reqParamsJsonJVar,
      ...
      JBlock methodTestBlk) {
        ...
        if (fields != null) {
            for (String fieldName : fields.keySet()) {
                ...
                JVar fieldNameVar = methodTestBlk.decl(jType, StringHelper.toLowerCaseFirstOne(
                  responseBodyVar.type().name()) + loopSeq + tterMethodName,
                  responseBodyVar.invoke(getter));
                methodTestBlk.add(
                  JExpr.ref("log").invoke("debug").arg(JExpr.lit(fieldNameVar.name() + "{}")).arg(
                    fieldNameVar));
                if (!canBeNull) {
                    methodTestBlk.add(
                      cmTest.ref("org.testng.Assert").staticInvoke("assertNotNull").arg(
                        fieldNameVar));
                }
                if (primitiveType == null && jType.name().equals("String")) {
                    ...
                    JBlock ifBlock2 =
                      ifBlock1._if(JExpr.ref("actualLength").gt(JExpr.lit(length)))._then();
                    ...
                    ifBlock2.add(cmTest.ref("org.testng.Assert").staticInvoke("fail").arg(
                      JExpr.lit(sbFailReason.toString()).plus(JExpr.ref("actualLength"))));
                }
    ...
    private void logResponseBody(int loopSeq, PacketObject responseBody, JVar responseBodyVar,
      JBlock methodTestBlk) {
        ...
        if (robjects != null && robjects.size() > 0) {
            for (String reName : robjects.keySet()) {
                ...
                JVar reNameVar =
                  methodTestBlk.decl(roClass, responseBody.getName() + tterMethodName + loopSeq,
    ...
    public void processClientTestPraram() {
        ...
        if (headVariables != null && headVariables.size() > 0) {
            for (int index = 0; index < headVariables.size(); index++) {
                PacketField headVariable = headVariables.get(index);
                methodParamInJsonObject.put(headVariable.getNameAlias(),
                  headVariable.getDatatype());
    ...
    public void processControllerMethod(CommonController commonController,
      JDefinedClass controllerClass) {
        ...
        this.jcontrollerMethod =
          controllerClass.method(JMod.PUBLIC, this.exchangeProcessor.getResponseClass(),
        ...
        JInvocation jInvocation = JExpr.invoke(JExpr.refthis(commonController.getServiceName()),
          this.serviceMethodGenerator.getMethodName());

        methodPath =
          methodPath + this.exchangeProcessor.processHTTPRequest(jcontrollerMethod, jInvocation,
            mockModel, this.cm);

        if (this.getMockModel() == MockModel.MockModel_Swagger) {
            this.jcontrollerMethod.annotate(cm.ref("io.swagger.annotations.ApiOperation")).param(
              "value", methodRemark).param("notes", methodRemark);
        }
        List<PacketField> headVariables =
          this.serviceMethodGenerator.getCommonMethod().getHeadVariables();
        if (headVariables != null && headVariables.size() > 0) {
            for (int index = 0; index < headVariables.size(); index++) {
                ...
                headVariableJVar.annotate(
                  cm.ref("org.springframework.web.bind.annotation.RequestHeader")).param("name",
                  headVariable.getName()).param("required", true);
                jInvocation.arg(headVariableJVar);
                if (this.getMockModel() == MockModel.MockModel_Swagger) {
                    headVariableJVar.annotate(cm.ref("io.swagger.annotations.ApiParam")).param(
                      "name", headVariable.getName()).param("value",
                      headVariable.getRemark()).param("required", true);
                }
            }
        }
        List<PacketField> pathVariables =
        ...
        if (pathVariables != null && pathVariables.size() > 0) {
            for (int index = 0; index < pathVariables.size(); index++) {
                ...
                if (this.getMockModel() == MockModel.MockModel_Swagger) {
                    pathVariableJVar.annotate(cm.ref("io.swagger.annotations.ApiParam")).param(
        ...
        if (parameters != null && parameters.size() > 0) {
            for (int index = 0; index < parameters.size(); index++) {
                ...
                parameterJVar.annotate(
                  cm.ref("org.springframework.web.bind.annotation.RequestParam")).param("name",
                  parameter.getName()).param("required", true);

                jInvocation.arg(parameterJVar);
                if (this.getMockModel() == MockModel.MockModel_Swagger) {
                    parameterJVar.annotate(cm.ref("io.swagger.annotations.ApiParam")).param("name",
                      parameter.getName()).param("value", parameter.getRemark()).param("required",
                      true);
                }
            }
        }
        JAnnotationUse jAnnotationUse = this.jcontrollerMethod.annotate(
        ...
        if (cookieVariables != null && cookieVariables.size() > 0) {
            for (int index = 0; index < cookieVariables.size(); index++) {
                ...
                cookieJVar.annotate(
                  cm.ref("org.springframework.web.bind.annotation.CookieValue")).param("name",
                  cookieVariable.getName()).param("required", true);
                jInvocation.arg(cookieJVar);
                if (this.getMockModel() == MockModel.MockModel_Swagger) {
                    cookieJVar.annotate(cm.ref("io.swagger.annotations.ApiParam")).param("name",
                      cookieVariable.getName()).param("value",
                      "cookie:" + cookieVariable.getRemark()).param("required", true);
                }
                controllerMethodBlk.add(
                  JExpr.ref("log").invoke("debug").arg(JExpr.lit(cookieJVar.name() + "{}")).arg(
                    cookieJVar));
            }
        }
        ...
        if (this.serviceMethodGenerator.getRequestBodyClass() != null) {
            JVar requestParam =
              this.jcontrollerMethod.param(this.serviceMethodGenerator.getRequestBodyClass(),
                "requestBody");
